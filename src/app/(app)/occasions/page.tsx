...
export default function OccasionsPage() {
    ...
    const [activeResult, setActiveResult] = useState<{ id: string, outfit: { items: WardrobeItem[] } | null, ai: { generated_text: string, weather_text: string } | null }>({ id: "", outfit: null, ai: null });
                            ...
                            {occasions.map((occ) => (
                                <div key={occ.occasion_id} className="bg-white/3 border border-white/10 p-6 rounded-3xl relative group hover:bg-white/5 transition-all duration-300">
                                    <button 
                                        ...
                                        className="absolute top-6 right-6 text-gray-600 hover:text-red-400 opacity-0 group-hover:opacity-10 transition-opacity p-2"
                                    >
